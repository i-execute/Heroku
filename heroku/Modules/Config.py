...
@loader.tds
class Config(loader.Module):
    """Interactive configurator"""

    strings = {
        "name": "Config",
        "choose_core": "<b>Choose a category</b>",
        "configuring_folder": "<b>Choose config option for folder</b> <code>{}</code>\n\n<b>Current options:</b>\n\n{}",
        "category_header": "<b> {}</b>",
        ...
        "try_again": "Try again",
        "typehint": "<b>Must be a {}</b>",
        "set_default_btn": "Reset default",
        "enter_value_btn": "Enter value",
        "enter_value_desc": "Enter new configuration value for this option",
        "add_item_desc": "Enter item to add",
        "remove_item_desc": "Enter item to remove",
        "back_btn": "Back",
        "close_btn": "Close",
        ...
        "_cmd_doc_cfg": "Configure modules",
        "set": "Set",
        ...
        "option_saved_lib": "<b>Option</b> <code>{}</code> <b>of library</b> <code>{}</code><b> saved!</b>\n<b>Current: {}</b>",
        "option_reset": "<b>Option</b> <code>{}</code> <b>of module</b> <code>{}</code> <b>has been reset to default</b>\n<b>Current: {}</b>",
        "option_reset_lib": "<b>Option</b> <code>{}</code> <b>of library</b> <code>{}</code> <b>has been reset to default</b>\n<b>Current: {}</b>",
        "configure": "<b>Choose a module to configure</b>",
        "configure_lib": "<b>Choose a library to configure</b>",
        "configuring_category": "<b>Choose config option for mod</b> <code>{}</code> <b>in category</b> <code>{}</code>\n<i>{}</i>\n\n<b>Current options:</b>\n{}",
        "configuring_category_lib": "<b>Choose config option for library</b> <code>{}</code> <b>in category</b> <code>{}</code>\n<i>{}</i>\n\n<b>Current options:</b>\n{}",
    ...
    def _paginate_text_markup(
        ...
        row = []
        if page > 0:
            row.append({"text": "", "callback": callback, "args": (page - 1,), "style": "primary"})
    ...
    async def inline__set_bool(
        ...
        doc = utils.escape_html(validator.doc["en"])
        ...
        await call.edit(
            self.strings[
                ...
                (
                    self.strings["typehint"].format(doc)
                    if doc
                    else ""
                ),
            ),
        ...
        return [
            ...
            *utils.chunks(
                [
                    {
                        "text": (
                            f"{'' if self.lookup(mod).config[option] == value else ''} "
        ...
        return [
            ...
            *utils.chunks(
                [
                    {
                        "text": (
                            f"{'' if value in self.lookup(mod).config[option] else ''} "
                            f"{value if len(str(value)) < 20 else str(value)[:20]}"
                        ),
    ...
    async def inline__configure(
        ...
        if category is not None:
            ...
            option_lines = [
                f" <code>{utils.escape_html(p)}</code>: {fmt_value(p)}"
                for p in params
            ]
            ...
            reply_markup=[
                [
                    ...
                    },
                    {
                        ...
                        "callback": self.inline__global_config, "style": "primary",
                    },
                ],
                *(
                    [
    ...
    async def inline__global_folder(
        ...
        for mod_name, params in folder_options.items():
            try:
                ...
                text_parts.append(
                    f" <b>{utils.escape_html(mod_name)}</b>"
                )
            except Exception:
                text_parts.append(
                    f" <b>{utils.escape_html(mod_name)}</b>"
    ...
    def _category_option(
        ...
        if option in Config._config_categories(instance).get(category, []):
            return option
    ...
    @loader.command()
    async def cfg(self, message: Message):
